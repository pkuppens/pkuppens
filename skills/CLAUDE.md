# skills/ — Agent Instructions

When working in this directory, follow these rules.

## Scope

This directory contains unified AI coding skills for Cursor, Claude, and Codex. Skills are committed to the repo and referenced via symlinks from IDE expected locations.

## Structure

- Each skill lives in its own directory with `SKILL.md`
- Skill tree overview: [REFACTORED_SKILLS.md](REFACTORED_SKILLS.md)
- Meta-skills (e.g. skill-creation) in `_meta/`

## Creating or editing skills

1. Use [skill-creation](_meta/skill-creation/SKILL.md) for new skills
2. Follow REFACTORED_SKILLS.md principles: single responsibility, one-liner, progressive disclosure
3. Conform to [Claude skill structure](https://docs.claude.com/en/docs/agents-and-tools/agent-skills/best-practices#skill-structure): YAML frontmatter (name, description), concise body, progressive disclosure
4. Keep SKILL.md under ~500 lines; use reference.md, examples.md for details

## Scratch

Scratch files (research, drafts) live in `tmp/skills/` at repo root, gitignored.
